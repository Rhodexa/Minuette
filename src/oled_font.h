...
// Printable-ASCII-only (0x20 ' ' through 0x7E '~', 95 glyphs) subset of the
// classic public-domain 5x7 font shipped with Adafruit_GFX (glcdfont.c) —
// trimmed down to just what oled.cpp's printer needs, skipping the extended
// Latin/box-drawing glyphs past 0x7E to keep this small.
//
// Each glyph is 5 bytes, one per column, top-aligned within a page: bit 0
// is the glyph's top pixel row, bit 6 its bottom row (bit 7 unused, since
// the font is 7px tall and a page is 8px) — the same layout oledWritePage()
// expects, so a glyph can be handed to it directly.
static const uint8_t OLED_FONT_FIRST_CHAR = 0x20;
static const uint8_t OLED_FONT_LAST_CHAR = 0x7E;
static const uint8_t OLED_FONT_GLYPH_WIDTH = 5;

static const uint8_t oledFont5x7[95][5] PROGMEM = {
  ...
  {0x7C, 0x09, 0x05, 0x05, 0x78}, // 0x7C 'ñ' // normally '|' {0x00, 0x00, 0x77, 0x00, 0x00},  Spanish patch
